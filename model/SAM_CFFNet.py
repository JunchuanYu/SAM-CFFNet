...
class SFE(nn.Module):
    ...
    def forward(self,input, mean_feature):
        x = self.down_for(input)
        x = self.atts(mean_feature, x)
        return x
...
class CFFD(nn.Module):
    ...
    def forward(self,input, outs):
        ...
        x = self.marges(torch.cat(outs, dim=1))
        x = F.interpolate(x, size=(128, 128), mode='bilinear', align_corners=True)

        s = self.sfe(input, x)
        
        x = self.last_layer(torch.cat((x, s), dim=1))
        return x
...
class SAM_CFFNet(nn.Module):
    ...
    def forward(self,input):
        input = F.interpolate(input, size=(1024, 1024), mode='bilinear', align_corners=True)
    ...
    def load_weight(self, sam_pretrained_weights):
        sam_model_list = torch.load(sam_pretrained_weights)
        image_encoder_submodules = {name[14:]: state for name, state in sam_model_list.items() if 'image_encoder' in name}
        self.image_encoder.load_state_dict(image_encoder_submodules, strict=False)
